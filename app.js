...
window.PFHApp=(function(){
 ...
 async function syncAll(){authUI();if(!PFHGoogle.isConnected()){PFHCalendar.render();PFHChores.render();return}try{let items=await PFHGoogle.list(); // one fetch, then each module can map its own data
   // calendar sync currently fetches independently to keep modules simple/reliable
   await PFHCalendar.sync();await PFHChores.sync(items);authUI()
 ...
 function boot(){clock();setInterval(clock,30000);PFHCalendar.init();PFHChores.init();pages();PFHGoogle.init();authUI();$("#connectionBtn").onclick=()=>PFHGoogle.isConnected()?syncAll():PFHGoogle.connect();window.addEventListener("pfh-auth",()=>{authUI();syncAll()});setTimeout(()=>{PFHGoogle.init();authUI();if(PFHGoogle.isConnected())syncAll()},700)}
