...
window.PFHApp=(function(){
 ...
 async function syncAll(){authUI();if(!PFHGoogle.isConnected()){PFHCalendar.render();PFHChores.render();PFHRecipes.render();PFHMeals.render();PFHLists.render();return}try{
   const items=await PFHGoogle.list();
   PFHCalendar.loadFromItems(items);
   PFHChores.loadFromItems(items);
   PFHRecipes.loadFromItems(items);
   PFHMeals.loadFromItems(items);
   PFHLists.loadFromItems(items);
   authUI();
 ...
 function boot(){clock();setInterval(clock,30000);PFHCalendar.init();PFHChores.init();PFHRecipes.init();PFHMeals.init();PFHLists.init();PFHWeather.init();pages();PFHGoogle.init();authUI();$("#connectionBtn").onclick=()=>PFHGoogle.isConnected()?syncAll():PFHGoogle.connect();window.addEventListener("pfh-auth",()=>{authUI();syncAll()});setTimeout(()=>{PFHGoogle.init();authUI();if(PFHGoogle.isConnected())syncAll()},700)}
